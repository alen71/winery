...
const WinesSliderButton = ({ side, action, color, className }: Props) => {
  return (
    <motion.div
      ...
      className={clsx(
        `absolute top-[50%] ${side}-0  overflow-hidden flex items-center z-10 ${className}`,
        {
          'w-[72px] h-44': color === 'white',
          'w-[52px] sm:w-[72px] h-32 sm:h-44': color === 'primary'
        }
      )}
    >
      <div
        className={clsx('absolute rotate-45 h-32 w-32  flex items-center', {
          ...
          'bg-gray-bg': color === 'primary',
          'bg-white/25': color === 'white'
        })}
      >
        <div
          className={clsx(
            `absolute  border-[1px] border-${color} text-${color} rounded-full flex justify-center items-center cursor-pointer`,
            {
              '-rotate-45 right-3 sm:right-4 top-3 sm:top-4': side === 'left',
              'rotate-[135deg] left-3 sm:left-4 bottom-3 sm:bottom-4':
                side === 'right',
              'w-7 sm:w-9 h-7 sm:h-9': color === 'primary',
              'w-9 h-9': color === 'white'
            }
          )}
          onClick={action}
